...
Post-hoc line plots for the OBSTACLE main effect:
  Contrast = Unexpected Present (UP)  vs  Expected Present (EP)
...
Per significant omnibus cluster (p < 0.05):
  ...
  2) Extract per-time ROI series for UP & EP (per subject)
  3) Run paired time-only cluster permutation on the UP-EP difference (two-sided)
...
PLOT_ROOT = Path(r"D:\Cluster_plots_last_prep\line_plots\obstacle_UP_vs_EP")
CSV_ROOT = Path(r"D:\cluster_outputs\line_posthocs\obstacle_UP_vs_EP")
...
def _collect_up_vs_ep(segment: str, prep_window: str = "last"):
    """
    Returns:
    ...
    per_subj_up, per_subj_ep = [], []
    freqs = None
    times = None

    for pid in participants:
        per_light_up, per_light_ep = [], []
...
def _plot_lines_with_shading(times, yA, yB, ciA, ciB, sig_windows,
                             seg, out_png,
                             title_suffix="Unexpected Present vs Expected Present",
                             label_A="UP", label_B="EP"):
...
def run_for_npz(npz_path, plots_dir: Path, csv_dir: Path):
    ...
    # Gather UP/EP arrays (mirrors omnibus collapse)
    up, ep, _, _ = _collect_up_vs_ep(
        segment=str(seg).upper(), prep_window="last")
    expected_shape = (len(participants), n_ch, n_f, n_t)
    if up.shape != expected_shape or ep.shape != expected_shape:
        raise RuntimeError(
            f"Data shapes {up.shape} / {ep.shape} != expected {expected_shape}")
    ...
    csv_path = seg_csv_dir / f"UP_vs_EP_{seg_upper}.csv"
    ...
    for k, (mask1d, pv) in enumerate(zip(masks, pvals)):
        ...
        meanUP, ciUP, meanEP, ciEP = _within_subject_ci_two_cond(
            y_up, y_ep, alpha=0.05)
        ...
        for r in rows:
            ...
            print(f"[{seg_upper} c{str(k).zfill(2)}] "
                  ...
                  f"CSV UP={csv_up:.2f}, CSV EP={csv_ep:.2f}  "
                  f"PlotPeak UP={peak_up:.2f}, PlotPeak EP={peak_ep:.2f}")

        out_png = seg_plot_dir / f"cluster{str(k).zfill(2)}_UP_vs_EP.png"
        ...
        _plot_lines_with_shading(times, meanUP, meanEP, ciUP, ciEP, rows_in_window, seg_upper, out_png,
                                 title_suffix="Unexpected Present vs Expected Present",
                                 label_A="Unexpected Present", label_B="Expected Present")
        ...
        with open(pkl_path, "wb") as fh:
            pickle.dump({
                ...
                "yA": meanUP, "yB": meanEP,
                "ciA": ciUP, "ciB": ciEP,
                "labelA": "Unexpected Present", "labelB": "Expected Present",
                "line_colors": ["#0D5C9D", "#1C7F46"],
                "box_color": "black",
        ...
        # ROI summary (channels + freq range) for this omnibus cluster — WITH NAMES
        ...
        for r in rows:
            ...
            eff = _window_effect_summary(
                y_up, y_ep, win_mask)  # mean_A=UP, mean_B=EP
            ...
            out_rows.append({
                ...
                # group means in the window (A=UP, B=EP)
