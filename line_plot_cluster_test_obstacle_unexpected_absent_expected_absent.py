...
Post-hoc line plots for the OBSTACLE main effect:
  Contrast = Unexpected Absent (UA)  vs  Expected Absent (EA)
...
Per significant omnibus cluster (p < 0.05):
  ...
  2) Extract per-time ROI series for UA & EA (per subject)
  3) Run paired time-only cluster permutation on the UA-EA difference (two-sided)
...
PLOT_ROOT = Path(r"D:\Cluster_plots_last_prep\line_plots\obstacle_UA_vs_EA")
CSV_ROOT = Path(r"D:\cluster_outputs\line_posthocs\obstacle_UA_vs_EA")
...
def _collect_ua_vs_ea(segment: str, prep_window: str = "last"):
    """
    Returns:
      ua  : (n_subj, n_ch, n_f, n_t)
      ea  : (n_subj, n_ch, n_f, n_t)
      times: (n_t,)
      freqs: (n_f,)
    """
    assert segment in ("PREP", "RESET")

    per_subj_ua, per_subj_ea = [], []
    freqs = None
    times = None

    for pid in participants:
        per_light_ua, per_light_ea = [], []

        for light in LIGHT_LEVELS:
            cond_EA = f"GLOBAL_{light}_EXPECTED_ABSENT"
            cond_UA = f"GLOBAL_{light}_UNEXPECTED_ABSENT"

            EA_prep, EA_reset, t_prep, t_reset, fr = load_and_extract_prep_reset(
            ...
            per_light_ea.append(arr_EA)
            per_light_ua.append(arr_UA)

        # collapse across LIGHT (mean of 3 lights)
        ea_pid = np.mean(np.stack(per_light_ea, axis=0), axis=0)
        ua_pid = np.mean(np.stack(per_light_ua, axis=0), axis=0)

        per_subj_ea.append(ea_pid)
        per_subj_ua.append(ua_pid)

    ua = np.stack(per_subj_ua, axis=0)
    ea = np.stack(per_subj_ea, axis=0)
    return ua, ea, times, freqs
...
def _plot_lines_with_shading(times, yA, yB, ciA, ciB, sig_windows,
                             seg, out_png,
                             title_suffix="Unexpected Absent vs Expected Absent",
                             label_A="Unexpected Absent", label_B="Expected Absent"):
...
def _cluster_roi_info(mask1d, n_ch, n_f, n_t, freqs, ch_names=None):
    ...
    return {
        ...
        "freq_min_hz": fmin,
        "freq_max_hz": fmax,
        "roi_channels_list": ";".join(ch_list),  
    }
...
def _window_effect_summary(yA, yB, win_mask):
    ...
    subj_A = yA[:, idx].mean(axis=1)  # UA
    subj_B = yB[:, idx].mean(axis=1)  # EA
    ...
    return {
        "mean_A": float(subj_A.mean()),  # UA
        "mean_B": float(subj_B.mean()),  # EA
        "diff_mean": diff_mean,
...
def run_for_npz(npz_path, plots_dir: Path, csv_dir: Path):
    ...
    # Gather UA/EA arrays (mirrors omnibus collapse)
    ua, ea, _, _ = _collect_ua_vs_ea(
        segment=str(seg).upper(), prep_window="last")
    expected_shape = (len(participants), n_ch, n_f, n_t)
    if ua.shape != expected_shape or ea.shape != expected_shape:
        raise RuntimeError(
            f"Data shapes {ua.shape} / {ea.shape} != expected {expected_shape}")
    ...
    csv_path = seg_csv_dir / f"UA_vs_EA_{seg_upper}.csv"
    ...
    for k, (mask1d, pv) in enumerate(zip(masks, pvals)):
        ...
        y_ua = _roi_timeseries_from_mask(
            ua, mask1d, n_ch, n_f, n_t)  # (n_subj, n_t)
        y_ea = _roi_timeseries_from_mask(
            ea, mask1d, n_ch, n_f, n_t)  # (n_subj, n_t)
        diff = y_ua - y_ea
        ...
        meanUA, ciUA, meanEA, ciEA = _within_subject_ci_two_cond(
            y_ua, y_ea, alpha=0.05)

        # print CSV-style vs plot-peak diagnostics
        for r in rows:
            m = (times >= r["time_start"]) & (times <= r["time_end"])
            csv_ua = y_ua[:, m].mean(axis=1).mean()
            csv_ea = y_ea[:, m].mean(axis=1).mean()
            peak_ua = meanUA[m].max()
            peak_ea = meanEA[m].max()
            print(f"[{seg_upper} c{str(k).zfill(2)}] "
                  f"{r['time_start']:.3f}–{r['time_end']:.3f}s  "
                  f"CSV UA={csv_ua:.2f}, CSV EA={csv_ea:.2f}  "
                  f"PlotPeak UA={peak_ua:.2f}, PlotPeak EA={peak_ea:.2f}")

        out_png = seg_plot_dir / f"cluster{str(k).zfill(2)}_UA_vs_EA.png"
        ...
        _plot_lines_with_shading(times, meanUA, meanEA, ciUA, ciEA, rows_in_window, seg_upper, out_png,
                                 title_suffix="Unexpected Absent vs Expected Absent",
                                 label_A="Unexpected Absent", label_B="Expected Absent")
        ...
        with open(pkl_path, "wb") as fh:
            pickle.dump({
                ...
                "yA": meanUA, "yB": meanEA,
                "ciA": ciUA, "ciB": ciEA,
                "labelA": "Unexpected Absent", "labelB": "Expected Absent",
                "line_colors": ["#0D5C9D", "#1C7F46"],
                "box_color": "black",
        ...
        for r in rows:
            ...
            eff = _window_effect_summary(
                y_ua, y_ea, win_mask)  # mean_A=UA, mean_B=EA
            ...
            out_rows.append({
                ...
                # group means in the window (A=UA, B=EA)
