...
Post-hoc line plots for the OBSTACLE main effect:
  Contrast = Unexpected Present (UP)  vs  ALL_ABSENT (mean of EA, UA)
...
Per significant omnibus cluster (p < 0.05):
  ...
  2) Extract per-time ROI series for UP & ALL_ABSENT (per subject)
  3) Run paired time-only cluster permutation on (UP - ALL_ABSENT), two-sided
...
PLOT_ROOT = Path(
    r"D:\Cluster_plots_last_prep\line_plots\obstacle_UP_vs_ALL_ABSENT")
CSV_ROOT = Path(r"D:\cluster_outputs\line_posthocs\obstacle_UP_vs_ALL_ABSENT")
...
def _collect_up_vs_all_absent(segment: str, prep_window: str = "last"):
    """
    Returns:
      up          : (n_subj, n_ch, n_f, n_t)
    ...
    per_subj_up, per_subj_allabs = [], []
    freqs = None
    times = None

    for pid in participants:
        per_light_up, per_light_allabs = [], []
        ...
        for light in LIGHT_LEVELS:
            ...
            UP_prep, UP_reset, t_prep, t_reset, fr = load_and_extract_prep_reset(
...
def _time_cluster_test(diff, alpha_cf=ALPHA_CF, n_perm=N_PERM, seed=SEED):
    """
    diff: (n_subj, n_time) UP - ALL_ABSENT
...
def _plot_lines_with_shading(times, yA, yB, ciA, ciB, sig_windows,
                             seg, out_png,
                             title_suffix="Unexpected Present vs Absent",
                             label_A="Unexpected Present", label_B="Absent"):
...
def _window_effect_summary(yA, yB, win_mask):
    ...
    Here, A=UP, B=ALL_ABSENT (mean of EA, UA).
    """
    idx = np.where(win_mask)[0]
    subj_A = yA[:, idx].mean(axis=1)  # UP
...
def run_for_npz(npz_path, plots_dir: Path, csv_dir: Path):
    ...
    # Gather UP / ALL_ABSENT arrays 
    up, all_absent, _, _ = _collect_up_vs_all_absent(
        segment=str(seg).upper(), prep_window="last")
    expected_shape = (len(participants), n_ch, n_f, n_t)
    if up.shape != expected_shape or all_absent.shape != expected_shape:
        raise RuntimeError(
            f"Data shapes {up.shape} / {all_absent.shape} != expected {expected_shape}")
    ...
    csv_path = seg_csv_dir / f"UP_vs_ALL_ABSENT_{seg_upper}.csv"
    ...
    for k, (mask1d, pv) in enumerate(zip(masks, pvals)):
        ...
        y_up = _roi_timeseries_from_mask(
            up,         mask1d, n_ch, n_f, n_t)  # (n_subj, n_t)
        ...
        meanUP, ciUP, meanALL, ciALL = _within_subject_ci_two_cond(
            y_up, y_all, alpha=0.05)

        # print CSV-style vs plot-peak diagnostics
        for r in rows:
            m = (times >= r["time_start"]) & (times <= r["time_end"])
            csv_up = y_up[:,  m].mean(axis=1).mean()
            csv_all = y_all[:, m].mean(axis=1).mean()
            peak_up = meanUP[m].max()
            peak_all = meanALL[m].max()
            print(f"[{seg_upper} c{str(k).zfill(2)}] "
                  f"{r['time_start']:.3f}–{r['time_end']:.3f}s  "
                  f"CSV UP={csv_up:.2f}, CSV ALL_ABS={csv_all:.2f}  "
                  f"PlotPeak UP={peak_up:.2f}, PlotPeak ALL_ABS={peak_all:.2f}")

        out_png = seg_plot_dir / \
            f"cluster{str(k).zfill(2)}_UP_vs_ALL_ABSENT.png"
        ...
        _plot_lines_with_shading(times, meanUP, meanALL, ciUP, ciALL, rows_in_window, seg_upper, out_png,
                                 title_suffix="Unexpected Present vs ALL_ABSENT",
                                 label_A="Unexpected Present", label_B="Absent")
        ...
        with open(pkl_path, "wb") as fh:
            pickle.dump({
                "times": times,
                "yA": meanUP, "yB": meanALL,
                "ciA": ciUP, "ciB": ciALL,
                "labelA": "Unexpected Present", "labelB": "Absent",
                "line_colors": ["#0D5C9D", "#1C7F46"],
                "box_color": "black",
                "sig_windows": rows_in_window,
                "seg": seg_upper,
                "contrast": "UP vs ALL_ABSENT",
            }, fh)
        ...
        for r in rows:
            ...
            # mean_A=UP, mean_B=ALL_ABSENT
            eff = _window_effect_summary(y_up, y_all, win_mask)
            ...
            out_rows.append({
                ...
                # group means in the window (A=UP, B=ALL_ABSENT)
    ...
    meta = dict(
        ...
        n_permutations=N_PERM,
        seed=SEED,
        # clusters that produced sig windows 
